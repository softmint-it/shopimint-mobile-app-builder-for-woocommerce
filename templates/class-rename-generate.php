...
add_filter('wp_generate_attachment_metadata', 'shopimint_dt_generate_attachment_metadata', 10, 2);
...
function shopimint_dt_generate_attachment_metadata($metadata, $attachment_id)
{
    ...
    foreach ($sizes as $size => $size_data) {
        $resized = shopimint_dt_image_make_intermediate_size($file, $size_data['width'], $size_data['height'], $size, $size_data['crop']);
        if ($resized)
 ...
 */
function shopimint_dt_image_make_intermediate_size($file, $width, $height, $size, $crop = false)
{
    if ($width || $height) {
        switch ($size) {
            case 'thumbnail':

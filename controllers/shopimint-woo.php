...
class ShopimintWoo extends ShopimintBaseController
{
    /**
     * Endpoint namespace
     *
    ...
    public function register_shopimint_woo_routes()
    {
		...
		register_rest_route($this->namespace, '/guestcheckout', array(
            array(
                'methods' => "POST",
                'callback' => array($this, 'guest_checkout'),
                'permission_callback' => function () {
                    return parent::checkApiPermission();
                }
            ),
		...
		register_rest_route($this->namespace, '/generate_guest_cart', array(
            array(
                'methods' => "GET",
                'callback' => array($this, 'generate_guest_cart'),
                'permission_callback' => function () {
                    return parent::checkApiPermission();
                }
            ),
        ...
        register_rest_route( $this->namespace,  '/getsmartbanner', array(
			array(
				'methods' => "GET",
				'callback' => array( $this, 'get_smartbanner' ),
				'permission_callback' => function () {
                    return parent::checkApiPermission();
                }
			),
        ...
        register_rest_route( $this->namespace,  '/smartbanner', array(
			array(
				'methods' => "POST",
				'callback' => array( $this, 'update_smartbanner' ),
				'permission_callback' => function () {
                    return parent::checkApiPermission();
                }
			),
        ...
        register_rest_route( $this->namespace,  '/abondendsend', array(
			array(
				'methods' => "POST",
				'callback' => array( $this, 'abondend_send_notifications' ),
				'permission_callback' => function () {
                    return parent::checkApiPermission();
                }
			),
        ...
        try {
            foreach ($products as $product) {
                ...
                if (!empty($variationId)) {
                    ...
                    shopimint_parseMetaDataForBookingProduct($product);
    ...
    public function get_cart($request)
    {
        ...
        foreach ($cart_items as $cart_item_key => $cart_item) {
            ...
            $items[] = ["cart_totals" => $cart_totals , "product" => $product_data, "quantity" => $quantity, "variation" => $variation_data];
        }
	...
	public function generate_guest_cart(){
		...
		$results = $wpdb->get_results("SELECT * FROM $table_name WHERE `code` = '$guest_id' ");
		
		$has_guestid = false;
		$cartitems = [];
		foreach( $results as $result ) {
			$cartitems = json_decode($result->order, true);
			$has_guestid = true;
		}
...
// 		wp_set_auth_cookie($guest_id);
        ...
        foreach ($products as $product) {
            ...
            // Check the product variation
            $current_productdata = wc_get_product($productId);
            if (!empty($variationId)) {
                
				$current_variation = wc_get_product($variationId);
				if ( ! $current_variation->has_enough_stock( $quantity ) ) {
					$outofstocklist[] = $this->get_cart_responce_product_data_($current_productdata , $request , $variationId , $quantity);
					...
					foreach ($listVariations as $vartiation => $value) {
						if ($variationId == $value['variation_id']) {
							...
							$instocklist[] = $this->get_cart_responce_product_data_($current_productdata , $request , $variationId , $quantity);
						}
					}
				}
				...
				$variation_id = 0;
				if ( ! $current_productdata->has_enough_stock( $quantity ) ) {
					$outofstocklist[] = $this->get_cart_responce_product_data_($current_productdata , $request , $variation_id , $quantity);
		
				} else {
					WC()->cart->add_to_cart($productId, $quantity, 0, $attributes);
					$instocklist[] = $this->get_cart_responce_product_data_($current_productdata , $request , $variation_id , $quantity);
				}
	...
	public function guest_checkout($request) {
		...
		global $wpdb;
		$guest_id = "";
		$generate_new_cart = true;
		$table_name = $wpdb->prefix . "shopimint_checkout";
        $productname = "";
		
		$cartitems = json_encode($body,JSON_FORCE_OBJECT); ;
		
		if (isset($body['guest_id']) && $body['guest_id'] != '0') {
			
			$guest_id = $body['guest_id'];
			$results = $wpdb->get_results("SELECT * FROM $table_name WHERE `code` = '$guest_id' ");
			foreach( $results as $result ) {
				$generate_new_cart = false;
				$wpdb->update($table_name, 
					array('order' => $cartitems),
					array('code'=>$guest_id)
				);
			}
		...
		if ($generate_new_cart) {
			$guest_id =  $code = md5(mt_rand() . strtotime("now"));
...
//         wp_set_auth_cookie($guest_id);
        ...
        foreach ($products as $product) {
            ...
            // Check the product variation
            $current_productdata = wc_get_product($productId);
            $productname = $productname.$current_productdata['name'].", ";

            if (!empty($variationId)) {
                
				$current_variation = wc_get_product($variationId);
				if ( ! $current_variation->has_enough_stock( $quantity ) ) {
					$stock = $this->get_cart_responce_product_data_($current_productdata , $request , $variationId , $quantity);
					$stock['instock'] = false;
					$instocklist[] = $stock;
				...
				$variation_id = 0;
				if ( ! $current_productdata->has_enough_stock( $quantity ) ) {
					$stock = $this->get_cart_responce_product_data_($current_productdata , $request , $variation_id , $quantity);
					$stock['instock'] = false;
					$instocklist[] = $stock;
		...
		$fcm = 'fcm_token';
		$user = [];
		$this->create_abonded_cart (false , $guest_id , $user , $cart_totals , $fcm, $stockqty, $productname );
		$cart_totals['guest_id'] = $guest_id;
		$cart_totals['stockqty'] = $stockqty;
		$cart_totals['out_of_stock_list'] = $outofstocklist;
		$cart_totals['instock_list'] = $instocklist;
		
        return $cart_totals;
	}

	
	public function create_abonded_cart ($isUser , $user_id , $user , $cart_totals , $fcm , $cartcount, $productname ) {
		
		global $wpdb;
		$table_name = $wpdb->prefix . "shopimint_abandoned_carts";
		
		if($isUser) {
			$results = $wpdb->get_results('SELECT * FROM '.$table_name.' WHERE cart_ignored = "0" AND recovered_cart = "0" AND user_id = '.$user_id);
		}else {
			$results = $wpdb->get_results('SELECT * FROM '.$table_name.' WHERE cart_ignored = "0" AND recovered_cart = "0" AND session_id = "'.$user_id.'" ');
		}
		...
		if($new_cart == 1){
			$wpdb->insert($table_name, array(
				'session_id' => $user_id,
				'user_type' => ($isUser) ? 'REGISTERED' : 'GUEST',
				'user_id' => $user_id,
				'fcm_token' => $fcm,
				...
				'checkout_link' => $user_id,
				'user_email_id' => ($isUser) ? $user->data->user_email : '',
			));
		}
		return true;
    ...
    public function sync_cart_from_mobile($request){
		...
		$fcm = get_user_meta( $user_id, 'fcm_token', true );
		$this->create_abonded_cart (true , $user_id , $user , $cart_totals , $fcm, $stockqty, $productname);
	...
	public function get_cart_responce_product_data_($current_productdata , $request , $variation_id , $quantity) {
		
		$product_controller = new WC_REST_Products_Controller();
        $product_variation_controller = new WC_REST_Product_Variations_Controller();
		$product_data = $product_controller->prepare_object_for_response($current_productdata, $request)->get_data();
		if ($variation_id != 0) {
			$variation = new WC_Product_Variation($variation_id);
			$variation_data = $product_variation_controller->prepare_object_for_response($variation, $request)->get_data();
			$attributes_array = [];
			foreach($variation_data['attributes'] as $variant) {
				$attributes_array[] = $variant['option'];
			}
			$variation_data['attributes'] = $attributes_array;
			$variation_data['image'] = (isset($variation_data['image']['src'])) ? $variation_data['image']['src'] : "";
		} else {
			$variation_data = null;
		}
		return ["product" => $product_data, "quantity" => $quantity, "variation" => $variation_data];
    ...
    public function upload_config_file($request){
        ...
        $errMsg = ShopimintUtils::upload_file_by_admin($file);
		if ($errMsg != null) {
			return parent::sendError("invalid_file","You need to upload config_xx.json file", 400);
		}
        return ShopimintUtils::get_json_file_url($file['name']);
	}
	...
	public function get_product_reviews($request) {
		...
		$args = array ('post_type' => 'product' , 'post_id' => $request['product_id']);
		...
		foreach ( $comments as $comment ) { 
			  ...
			  $review = [
				  ...
				  "date_created_gmt" => $comment->comment_date_gmt,
				  "product_id" => $comment->comment_post_ID,
				  "product_name" => "",
				  "product_permalink" => "",
				  "status" => "approved",
				  "reviewer" => get_comment_author( $comment ),
				  "reviewer_email" => "",
				  "review" => $comment->comment_content, 
				  "rating" => intval( get_comment_meta( $comment->comment_ID, 'rating', true ) ),
				  "verified" => false,
				  "reviewer_avatar" => get_avatar($comment, '60'),
				  "images" => $image_arr,
			  ];
    ...
    function check_product($request){
        ...
        if(!empty($cookie)){
            $userid = shopimint_validateCookieLogin($cookie);
		...
		foreach ($all_categories as $cat) {
			...
			if($cat->category_parent == 0) {
				...
				if($sub_cats) {
					...
					array_unshift($sub_cats, $showall);
				}
    ...
    public function get_smartbanner($request)
    {
        $args = array(
            'shopimint_smartbanner_enabled' => get_option("shopimint_smartbanner_enabled"),
            'shopimint_appname' => get_option("shopimint_appname"),
            'shopimint_appdescription' => get_option("shopimint_appdescription"),
            'shopimint_appicon' => get_option("shopimint_appicon"),
            'shopimint_ios_deeplink' => get_option("shopimint_ios_deeplink"),
            'shopimint_android_deeplink' => get_option("shopimint_android_deeplink"),
            'smartbanner_bgcolor' => get_option("smartbanner_bgcolor"),
            'smartbanner_textcolor' => get_option("smartbanner_textcolor"),
			'shopimint_plugin_enabled_time' => get_option("shopimint_plugin_enabled_time"),
			'shopimint_plugin_removed_time' => get_option("shopimint_plugin_removed_time"),
			'shopimint_appid' => get_option("shopimint_appid"),
        );
        return $args;
    }

    public function update_smartbanner($request)
    {
		$enabled = $request['shopimint_smartbanner_enabled'];
        $appname = $request['shopimint_appname'];
        $description = $request['shopimint_appdescription'];
        $appicon = $request['shopimint_appicon'];
        $iosdeeplink = $request['shopimint_ios_deeplink'];
        ...
        update_option("shopimint_appicon", $appicon);
        update_option("shopimint_ios_deeplink", $iosdeeplink);
        update_option("shopimint_android_deeplink", $androiddeeplink);
        update_option("smartbanner_bgcolor", $bgcolor);
        update_option("smartbanner_textcolor", $textcolor);
    ...
    public function abondend_send_notifications($request) {
        $appid = $request['appid'];
        $step = $request['step'];
		$response = abandonedCartNotification($step);
        return $response;
    }
